...
library(png)
library(grid)
img <- readPNG("ShinyGeneralWorkflow.PNG")
grid.raster(img)
```
...
shinyApp(ui = ui, server = server)
"
writeClipboard(gsub("'","\"",whatToCopy)
)
    
  })
}

shinyApp(ui = ui, server = server,options = list(height = 40))
```
...
ui <- fluidPage(
  ...
  fluidRow(
    column(width = 4,
          passwordInput(inputId = "answer.pass", label = NULL, value = "",
